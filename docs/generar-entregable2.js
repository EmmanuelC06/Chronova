...
const t = (text, o = {}) => new TextRun({
  text, font: FUENTE, size: o.size ?? 22, bold: o.bold, italics: o.italics, color: o.color,
});
...
const h1 = (text) => new Paragraph({
  ...
  children: [new TextRun({ text, font: FUENTE, size: 32, bold: true, color: '1A1A1A' })],
});
...
const h3 = (text) => new Paragraph({
  ...
  children: [new TextRun({ text, font: FUENTE, size: 23, bold: true, color: VERDE })],
});
...
function diagrama(archivo, anchoPx, altoPx, pie, opciones = {}) {
  ...
  const contenido = [
    ...
    new Paragraph({
      ...
      children: [t(pie, { size: 18, italics: true })],
    }),
  ];
...
function celda(texto, { anchura, cabecera = false, alterna = false, centrar = false, mono = false, size = 18 }) {
...
function tabla(columnas, cabeceras, filas, opciones = {}) {
  const { centrar = [], mono = [], size = 18 } = opciones;
  const bordes = {
...
/** Hueco para que Emmanuel pegue una captura de pantalla. */
function hueco(texto) {
  return new Table({
    columnWidths: [ANCHO_CONTENIDO],
    width: { size: ANCHO_CONTENIDO, type: WidthType.DXA },
    borders: {
      top: { style: BorderStyle.DASHED, size: 6, color: 'A0530A' },
      bottom: { style: BorderStyle.DASHED, size: 6, color: 'A0530A' },
      left: { style: BorderStyle.DASHED, size: 6, color: 'A0530A' },
      right: { style: BorderStyle.DASHED, size: 6, color: 'A0530A' },
    },
    rows: [new TableRow({
      children: [new TableCell({
        width: { size: ANCHO_CONTENIDO, type: WidthType.DXA },
        shading: { type: ShadingType.CLEAR, fill: 'FBF1E6' },
        margins: { top: 700, bottom: 700, left: 160, right: 160 },
        children: [new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [t(texto, { size: 18, bold: true, color: '7A3F06' })],
        })],
      })],
    })],
  });
...
const STACK = [
  ...
  ['Base de datos', 'PostgreSQL 16, alojada en Neon',
   'Los datos son fuertemente relacionales —un paciente tiene medicamentos, que generan tomas— y las reglas de integridad importan: una toma sin medicamento no debe existir. PostgreSQL ofrece además tipos que el modelo aprovecha: arreglos para los horarios y JSONB para permisos y preferencias, que pueden crecer sin migrar la tabla.'],
  ...
  ['Pruebas', 'Vitest',
   'Rápido y con la misma configuración de TypeScript del proyecto. Las 185 pruebas corren en menos de seis segundos porque la arquitectura permite ejecutarlas sin base de datos ni servidor.'],
...
const DIC_PACIENTES = [
  ['id', 'UUID', 'Identificador único del paciente', 'PK', '—'],
  ['nombre', 'TEXT', 'Nombre completo', 'NOT NULL', '—'],
  ['email', 'TEXT', 'Correo con el que inicia sesión', 'NOT NULL, UNIQUE', '—'],
  ['telefono', 'TEXT', 'Teléfono de contacto', 'Opcional', '—'],
  ['fecha_de_nacimiento', 'DATE', 'Fecha de nacimiento; permite calcular la edad', 'Opcional', '—'],
  ['contrasena_cifrada', 'TEXT', 'Contraseña cifrada con bcrypt. Nunca se guarda en texto claro', 'NOT NULL', '—'],
  ['zona_horaria', 'TEXT', 'Identificador IANA, por ejemplo America/Bogota', "NOT NULL, DEFAULT 'America/Bogota'", '—'],
  ['preferencias', 'JSONB', 'Tamaño de letra, contraste, alertas y minutos de gracia', "NOT NULL, DEFAULT '{}'", '—'],
  ['activo', 'BOOLEAN', 'Si la cuenta sigue vigente', 'NOT NULL, DEFAULT TRUE', '—'],
  ['creado_en', 'TIMESTAMPTZ', 'Fecha de creación de la cuenta', 'NOT NULL, DEFAULT NOW()', '—'],
  ['sesiones_validas_desde', 'TIMESTAMPTZ', 'Desde cuándo se aceptan sus tokens; cambiar la contraseña la mueve', 'NOT NULL, DEFAULT NOW()', '—'],
  ['politica_version', 'TEXT', 'Versión de la política de datos que aceptó', 'Nulo si no consta', '—'],
  ['politica_aceptada_en', 'TIMESTAMPTZ', 'Cuándo la aceptó; es la prueba de la autorización', 'Nulo si no consta', '—'],
];

const DIC_CUIDADORES = [
  ['id', 'UUID', 'Identificador único del cuidador', 'PK', '—'],
  ['nombre', 'TEXT', 'Nombre completo', 'NOT NULL', '—'],
  ['email', 'TEXT', 'Correo con el que inicia sesión', 'NOT NULL, UNIQUE', '—'],
  ['telefono', 'TEXT', 'Teléfono de contacto', 'Opcional', '—'],
  ...
  ['politica_version', 'TEXT', 'Versión de la política de datos aceptada', 'Nulo si no consta', '—'],
  ['politica_aceptada_en', 'TIMESTAMPTZ', 'Cuándo la aceptó', 'Nulo si no consta', '—'],
];

const DIC_MEDICAMENTOS = [
  ['id', 'UUID', 'Identificador único del medicamento', 'PK', '—'],
  ['paciente_id', 'UUID', 'Paciente al que pertenece', 'FK, NOT NULL, ON DELETE CASCADE', 'pacientes(id)'],
  ['nombre', 'TEXT', 'Nombre del medicamento', 'NOT NULL', '—'],
  ['dosis_cantidad', 'NUMERIC(10,2)', 'Cantidad por toma', 'NOT NULL, CHECK > 0', '—'],
  ['dosis_unidad', 'TEXT', 'Unidad de la dosis (tableta, ml, gota…)', 'NOT NULL', '—'],
  ['frecuencia_tipo', 'TEXT', 'DIARIA, DIAS_DE_LA_SEMANA o CADA_N_DIAS', 'NOT NULL, CHECK', '—'],
  ['frecuencia_dias', 'SMALLINT[]', 'Días de la semana cuando aplica (0 = domingo)', "NOT NULL, DEFAULT '{}'", '—'],
  ['frecuencia_intervalo', 'INTEGER', 'Cada cuántos días, cuando aplica', 'NOT NULL, DEFAULT 1', '—'],
  ['horarios', 'TEXT[]', 'Horas de toma del día, en formato de 24 horas', 'NOT NULL', '—'],
  ['fecha_inicio', 'DATE', 'Primer día del tratamiento', 'NOT NULL', '—'],
  ['fecha_fin', 'DATE', 'Último día, si el tratamiento tiene término', 'Opcional', '—'],
  ['instrucciones', 'TEXT', 'Indicaciones adicionales (con alimentos, etc.)', 'Opcional', '—'],
  ['stock_unidades', 'INTEGER', 'Unidades disponibles en inventario', 'NOT NULL, CHECK >= 0', '—'],
  ['stock_umbral', 'INTEGER', 'Umbral que dispara el aviso de reabastecimiento', 'NOT NULL, CHECK >= 0', '—'],
  ['activo', 'BOOLEAN', 'Falso cuando el tratamiento se suspende', 'NOT NULL, DEFAULT TRUE', '—'],
  ['creado_en', 'TIMESTAMPTZ', 'Fecha de registro', 'NOT NULL, DEFAULT NOW()', '—'],
  ['actualizado_en', 'TIMESTAMPTZ', 'Última modificación', 'NOT NULL, DEFAULT NOW()', '—'],
];

const DIC_TOMAS = [
  ['id', 'UUID', 'Identificador único de la toma', 'PK', '—'],
  ...
  ['programada_para', 'TIMESTAMPTZ', 'Hora vigente, que se corre al posponer', 'NOT NULL', '—'],
  ['programada_originalmente_para', 'TIMESTAMPTZ', 'Hora original; es contra esta que se mide la puntualidad', 'NOT NULL, UNIQUE con medicamento_id', '—'],
  ['estado', 'TEXT', 'PENDIENTE, POSPUESTA, TOMADA u OMITIDA', 'NOT NULL, CHECK', '—'],
  ['resuelta_en', 'TIMESTAMPTZ', 'Instante en que se confirmó u omitió', 'Nulo mientras esté pendiente', '—'],
  ['origen_del_registro', 'TEXT', 'Quién la registró: PACIENTE, CUIDADOR o SISTEMA', 'CHECK', '—'],
  ['registrada_por_id', 'UUID', 'Identificador de quien la registró', 'Sin FK (puede ser de dos tablas)', '—'],
  ['observaciones', 'TEXT', 'Nota libre asociada al registro', 'Opcional', '—'],
  ['veces_pospuesta', 'INTEGER', 'Cuántas veces se aplazó; el máximo es tres', 'NOT NULL, DEFAULT 0', '—'],
];

const DIC_VINCULOS = [
  ['id', 'UUID', 'Identificador único del vínculo', 'PK', '—'],
  ...
  ['estado', 'TEXT', 'PENDIENTE, ACEPTADO, RECHAZADO o REVOCADO', 'NOT NULL, CHECK', '—'],
  ['parentesco', 'TEXT', 'Relación declarada entre las dos personas', 'Opcional', '—'],
  ['permisos', 'JSONB', 'Los cuatro permisos que el paciente concede o niega', "NOT NULL, DEFAULT '{}'", '—'],
  ['solicitado_por', 'TEXT', 'Quién inició la solicitud: PACIENTE o CUIDADOR', 'NOT NULL, CHECK', '—'],
  ['creado_en', 'TIMESTAMPTZ', 'Fecha de la solicitud', 'NOT NULL, DEFAULT NOW()', '—'],
  ['resuelto_en', 'TIMESTAMPTZ', 'Fecha en que se aceptó, rechazó o revocó', 'Opcional', '—'],
];
...
const DIC_DISPOSITIVOS = [
  ...
  ['tipo_de_propietario', 'TEXT', 'PACIENTE o CUIDADOR; resuelve a qué tabla apunta', 'NOT NULL, CHECK', '—'],
  ['token', 'TEXT', 'Identificador de notificaciones del aparato', 'NOT NULL, UNIQUE', '—'],
  ['plataforma', 'TEXT', 'android, ios o web', 'NOT NULL, CHECK', '—'],
  ['registrado_en', 'TIMESTAMPTZ', 'Cuándo se registró el aparato', 'NOT NULL, DEFAULT NOW()', '—'],
  ['ultimo_uso_en', 'TIMESTAMPTZ', 'Último aviso entregado con éxito', 'NOT NULL, DEFAULT NOW()', '—'],
];

const DIC_RECUPERACIONES = [
  ['id', 'UUID', 'Identificador único de la solicitud', 'PK', '—'],
  ['usuario_id', 'UUID', 'Persona que solicita recuperar el acceso', 'NOT NULL, sin FK', 'pacientes o cuidadores'],
  ['tipo_de_cuenta', 'TEXT', 'PACIENTE o CUIDADOR', 'NOT NULL, CHECK', '—'],
  ['codigo_cifrado', 'TEXT', 'Código de seis dígitos, cifrado como una contraseña', 'NOT NULL', '—'],
  ['creada_en', 'TIMESTAMPTZ', 'Instante de la solicitud', 'NOT NULL', '—'],
  ['expira_en', 'TIMESTAMPTZ', 'Caducidad, treinta minutos después', 'NOT NULL', '—'],
  ['intentos', 'INTEGER', 'Intentos fallidos; el máximo es cinco', 'NOT NULL, DEFAULT 0', '—'],
  ['usada_en', 'TIMESTAMPTZ', 'Instante en que se usó; sirve una sola vez', 'Nulo mientras esté vigente', '—'],
];
...
function entradaUnica(filas) {
  return filas.map(([metodo, ruta, descripcion, parametros, cuerpo, respuesta]) => {
    const partes = [parametros, cuerpo].filter((v) => v && v !== '—');
    if (partes.length === 2) {
      throw new Error(`La ruta ${ruta} declara parámetros y body a la vez; hay que revisar la fusión de columnas.`);
    }
    return [metodo, ruta, descripcion, partes[0] ?? '—', respuesta];
  });
}

const ENDPOINTS_AUTH = [
  ['POST', '/api/auth/registro/paciente', 'Crear cuenta de paciente', '—', 'nombre, email, contrasena, zonaHoraria, aceptaPoliticaDeDatos', '201: token y usuario · 409: correo ya registrado'],
  ['POST', '/api/auth/registro/cuidador', 'Crear cuenta de cuidador', '—', 'nombre, email, contrasena, rol, aceptaPoliticaDeDatos', '201: token y usuario · 409: correo ya registrado'],
  ['POST', '/api/auth/sesion', 'Iniciar sesión', '—', 'email, contrasena, tipo', '200: token · 401: credenciales inválidas'],
  ['GET', '/api/auth/perfil', 'Perfil de quien tiene la sesión abierta', '—', '—', '200: perfil · 401: sin token'],
  ['PATCH', '/api/auth/preferencias', 'Ajustar accesibilidad', '—', 'tamanoDeLetra, alertas, minutosDeGracia', '200: preferencias actualizadas'],
  ['POST', '/api/auth/recuperacion', 'Pedir un código de recuperación', '—', 'email, tipo', '202: siempre, exista o no la cuenta'],
  ['POST', '/api/auth/recuperacion/confirmar', 'Cambiar la contraseña con el código', '—', 'email, codigo, nuevaContrasena', '200: contraseña cambiada · 422: código inválido'],
  ['POST', '/api/auth/dispositivos', 'Registrar el teléfono para avisos', '—', 'token, plataforma', '200: dispositivo registrado'],
  ['DELETE', '/api/auth/dispositivos', 'Dar de baja el teléfono', '—', 'token', '204: dado de baja'],
];

const ENDPOINTS_MED = [
  ['GET', '/api/medicamentos', 'Listar los medicamentos de un paciente', 'pacienteId, incluirSuspendidos', '—', '200: lista · 403: sin permiso'],
  ['POST', '/api/medicamentos', 'Registrar un medicamento', '—', 'nombre, dosis, frecuencia, horarios, fechaInicio, stock', '201: medicamento creado · 422: regla incumplida'],
  ['PATCH', '/api/medicamentos/:id', 'Modificar un medicamento', '—', 'campos a cambiar', '200: actualizado · 404: no existe'],
  ['DELETE', '/api/medicamentos/:id', 'Suspender un medicamento (no lo borra)', '—', '—', '200: suspendido, historial conservado'],
  ['POST', '/api/medicamentos/:id/stock', 'Reabastecer el inventario', '—', 'unidades', '200: inventario actualizado'],
];

const ENDPOINTS_TOMAS = [
  ['GET', '/api/tomas/agenda', 'Agenda de tomas de un día', 'fecha, pacienteId', '—', '200: agenda y resumen · 403: sin permiso'],
  ['POST', '/api/tomas/:id/registro', 'Confirmar, omitir o posponer una toma', '—', 'accion, observaciones, minutos', '200: toma registrada · 422: aún no es su hora'],
  ['GET', '/api/tomas/historial', 'Historial y adherencia de un período', 'desde, hasta, medicamentoId, pacienteId', '—', '200: registros, resumen y serie por día'],
];

const ENDPOINTS_VINC = [
  ['POST', '/api/vinculos', 'Solicitar un vínculo; lo inicia cualquiera de las dos partes', '—', 'emailDeLaOtraParte, parentesco', '201: solicitud creada · 404: correo no registrado'],
  ['POST', '/api/vinculos/:id/respuesta', 'Aceptar, rechazar o revocar; solo el paciente decide', '—', 'respuesta', '200: vínculo actualizado · 403: no es su vínculo'],
  ['PATCH', '/api/vinculos/:id/permisos', 'Cambiar los permisos de un cuidador', '—', 'los cuatro permisos', '200: permisos actualizados'],
  ['GET', '/api/cuidadores/pacientes', 'Panel del cuidador', 'dias', '—', '200: pacientes ordenados por urgencia · 403: token de paciente'],
  ['GET', '/api/pacientes/cuidadores', 'Quiénes acompañan al paciente', '—', '—', '200: cuidadores y sus permisos'],
];
...
const doc = new Document({
  ...
  sections: repartirEnSecciones([
      ...
      new Paragraph({
        ...
        children: [t('CHRONOVA', { size: 40, bold: true, color: VERDE })],
      }),
      new Paragraph({
      ...
      new Paragraph({
        alignment: AlignmentType.CENTER, spacing: { after: 200 },
        children: [
          t('Universidad Católica Luis Amigó', { size: 22 }),
          new TextRun({ text: 'Facultad de Ingenierías y Arquitectura', font: FUENTE, size: 22, break: 1 }),
          new TextRun({ text: 'Medellín, Colombia', font: FUENTE, size: 22, break: 1 }),
          new TextRun({ text: '2026', font: FUENTE, size: 22, break: 1 }),
        ],
      }),
      salto(),
      ...
      p('La afirmación es verificable, y esa es la razón de haberla elegido. En el directorio del dominio hay treinta y nueve archivos y 3.164 líneas, y ninguno importa una sola librería externa; la capa de infraestructura, en cambio, importa express, pg, bcryptjs, jsonwebtoken y zod. La consecuencia práctica es que la aplicación completa se ejecuta sin base de datos: basta con una variable de entorno para que los mismos casos de uso trabajen contra repositorios en memoria. Es también la razón de que las 185 pruebas automatizadas terminen en menos de seis segundos.'),
      ...
      tabla([1900, 3500, 3360],
        ...
        [
          ...
          ['Aplicación', 'Veinticuatro casos de uso y la PoliticaDeAcceso', 'Nada de negocio: orquesta. Pide la entidad, le manda hacer algo y la guarda'],
      ...
      h3('Tabla: pacientes'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_PACIENTES, { size: 16 }),
      aire(),
      h3('Tabla: cuidadores'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_CUIDADORES, { size: 16 }),
      aire(),
      h3('Tabla: medicamentos'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_MEDICAMENTOS, { size: 16 }),
      aire(),
      h3('Tabla: tomas'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_TOMAS, { size: 16 }),
      aire(),
      h3('Tabla: vinculos'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_VINCULOS, { size: 16 }),
      aire(),
      h3('Tabla: dispositivos'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_DISPOSITIVOS, { size: 16 }),
      aire(),
      h3('Tabla: recuperaciones'),
      tabla(DIC_COLUMNAS, ['Campo', 'Tipo', 'Descripción', 'Restricciones', 'Relación'], DIC_RECUPERACIONES, { size: 16 }),
      aire(),
      ...
      p([t('Errores. ', { bold: true }),
         t('Todos responden con la misma forma —un objeto error con código, mensaje y, si aplica, el campo afectado— y el mensaje está redactado en español para mostrarse tal cual al usuario.')]),

      h2('3.2  Endpoints principales'),
      h3('Autenticación y cuenta'),
      tabla(ENDPOINT_COLUMNAS, ENDPOINT_CABECERAS, entradaUnica(ENDPOINTS_AUTH), { size: 15, mono: [1] }),
      aire(),
      h3('Medicamentos'),
      tabla(ENDPOINT_COLUMNAS, ENDPOINT_CABECERAS, entradaUnica(ENDPOINTS_MED), { size: 15, mono: [1] }),
      aire(),
      h3('Tomas y adherencia'),
      tabla(ENDPOINT_COLUMNAS, ENDPOINT_CABECERAS, entradaUnica(ENDPOINTS_TOMAS), { size: 15, mono: [1] }),
      aire(),
      h3('Vinculación con cuidadores'),
      tabla(ENDPOINT_COLUMNAS, ENDPOINT_CABECERAS, entradaUnica(ENDPOINTS_VINC), { size: 15, mono: [1] }),
      aire(),
      ...
      p('La validación ocurre en dos niveles, y la distinción es deliberada. En el borde de la aplicación, esquemas de Zod comprueban la forma de los datos —que el campo exista, que sea texto, que el número sea entero— y devuelven mensajes en español. En el dominio, las entidades y los objetos de valor imponen las reglas de negocio, que no se pueden rodear llamando a la API directamente.'),
      ...
      ...diagrama('06-secuencia-agenda-del-dia.png', 1654, 1085,
        ...
        { apaisado: true,
          despues: [pFigura('El segundo muestra cómo un patrón —«una pastilla a las 8:00 todos los días»— se convierte en tomas concretas que el paciente puede confirmar, y cómo se resuelve el caso en que dos peticiones piden la agenda del mismo día simultáneamente.')] }),
      ...
      ...PANTALLAS.flatMap((pan, i) => [
        ...
        hueco(`[ Insertar aquí la captura de la pantalla «${pan[0]}» ]`),
        aire(120),
        p([t('Descripción. ', { bold: true }), t(pan[1])]),
        p([t('Componentes principales. ', { bold: true }), t(pan[2])]),
        p([t('Acciones posibles. ', { bold: true }), t(pan[3])]),
      ]),
      ...
      aire(),
      p('Dos decisiones merecen justificación aparte. La tipografía Atkinson Hyperlegible Next fue diseñada por el Braille Institute con el objetivo explícito de aumentar la legibilidad para personas con baja visión: sus letras se distinguen deliberadamente unas de otras —el cero lleva un corte, la ele tiene cola— justo donde otras familias las confunden. Y el texto base de 18 puntos, frente a los 14 habituales, responde al mismo criterio que las zonas táctiles de 64 píxeles.'),
      ...
      bloque([
        ...
        '│',
        '├── application/',
        '│   ├── use-cases/           24 casos de uso, agrupados por módulo',
        ...
        '',
        'backend/tests/               185 pruebas: dominio, casos de uso y HTTP',
      ]),
      aire(),
