...
export interface EntornoDePrueba {
  ...
  /**
   * Los repositorios, para las pruebas que necesitan mirar o tocar el
   * estado guardado directamente (por ejemplo desactivar una cuenta,
   * que todavia no tiene caso de uso propio).
   */
  pacientes: RepositorioDePacientesEnMemoria;
  cuidadores: RepositorioDeCuidadoresEnMemoria;
}

/**
 * Monta la aplicacion completa en memoria.
 *
...
export function montarAplicacion(
  ...
  const pacientes = new RepositorioDePacientesEnMemoria();
  const cuidadores = new RepositorioDeCuidadoresEnMemoria();
...
export async function crearPacienteDePrueba(
  ...
  const resultado =
    await entorno.contenedor.casosDeUso.registrarPaciente.ejecutar({
  ...
  return {
    ...
    solicitante: comoPaciente(resultado.usuario.id),
  };
}
...
export async function crearCuidadorDePrueba(
  ...
  const resultado =
    await entorno.contenedor.casosDeUso.registrarCuidador.ejecutar({
      nombre: "Ana Correa",
      email,
      contrasena: "contrasena-segura",
      rol: "Hija",
  ...
  return {
    id: resultado.usuario.id,
    solicitante: comoCuidador(resultado.usuario.id),
  };
}
